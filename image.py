...
class Image(object):
	...
	def render(self):
		new=bytearray()
		for i in range(300):
			new=new+bytearray(self.buffer[(i)*2000+i+1:(i+1)*2000+i+1])#Need this so not slanted and correct colour
		i = np.asarray( bytearray(new), dtype=np.uint8 ).reshape( 300, 500, 4 )
		cv2.imshow("Virtual Reality Scene", i)
